...
\end{equation}
The wake velocity depends on the downwind distance $(x)$ as well. As shown in Eq \ref{eq:Uw}, is the wake velocity an argument of the wake decay coefficient $c(x,y)$(<--) and the axial induction factor $a_i$. This wake decay coefficient describes the decay of velocity for each wake zone and is defined in \ref{eq:c}. The model parameter $M_{U,q}$, which depends on the wake zone and the yaw angle, is calculated by Gebraad.\cite{Gebraad2016}. The axial induction factor is a value for the wind speed reduction behind a turbine relative to its own rotor speed.    
...
FAST uses external input files as input for the simulation data. This paper mainly used the flow field as input towards FAST.
more intro:... making it more representative for real life, by implementing both a more realistic flow distribution and natural fenomena.
